...
#--initialize parameters to be used : for test purpose only
...
#---read in parameters and reset above parameters: cm, s are used 

f1=open("./beam_parameters_Teng.txt", 'r')
lines=f1.readlines()
f1.close()
...
def growth_rate( oneline ):
    ...
    name   = data[1]
    s      = data[2] * 100
    beta1  = float( data[3] ) * 100  
    alfa1  = float( data[4] ) 
    beta2  = float( data[5] ) * 100 
    alfa2  = float( data[6] ) 
    c11    = float( data[7] ) 
    c12    = float( data[8] ) * 100
    c21    = float( data[9] ) / 100
    c22    = float( data[10] )  
    dx     = float( data[11])  * 100
    dxp    = float( data[12]) 
    dy     = float( data[13])  * 100
    dyp    = float( data[14])
    q1     = float( data[15])
    q2     = float( data[16])

    rr     = np.sqrt( 1.0  - (c11 * c22 - c12 * c21 ) )

    #---matrix V, U

    V= np.array([
        [ rr * np.sqrt(beta1), 0 , c11 * np.sqrt(beta2) - c12 * alfa2 / np.sqrt(beta2),   c12 / np.sqrt(beta2)],
        [ -alfa1 * rr / np.sqrt(beta1),  rr / np.sqrt(beta1), c21 * np.sqrt(beta2) - c22 * alfa2 / np.sqrt(beta2),  c22 / np.sqrt(beta2) ],
        [-c12 * alfa1 /np.sqrt(beta1) - c22 * np.sqrt(beta1), c12 /np.sqrt(beta1), rr * np.sqrt(beta2), 0 ],
        [ c11*alfa1 / np.sqrt(beta1) + c21 * np.sqrt(beta1) , -c11 / np.sqrt(beta1), -alfa2 * rr/ np.sqrt(beta2),  rr /np.sqrt(beta2) ] 
        ])
    ...
    """
    rmin  = ru * c * c / sigmavtot / sigmavtot   
    ...
    v1 = np.array([ V[0,0]- 1j*V[0,1],  V[1,0] - 1j*V[1,1],  V[2,0] - 1j*V[2,1],   V[3,0] -1j*V[3,1] ] );
    v2 = np.array([ V[0,2]- 1j*V[0,3],  V[1,2] - 1j*V[1,3],  V[2,2] - 1j*V[2,3],   V[3,2] -1j*V[3,3] ] );
...
for  i in range( Ninteg ):
    ...
    print( dt *(i+1)/3600., emit1/100, emit2/100, sigmas/10, sigmap, tao1, tao2, tao3 )
